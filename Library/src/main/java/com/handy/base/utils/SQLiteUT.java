...
import com.ljcode.mybase.object.SQLTable;
 ...
 */
public class SQLiteUT {
    private static SQLiteUT sqLiteUT = null;

    private Context context;
    private Cursor cursor; //查询游标对象
    private int DB_VERSION = 1; //数据库版本
    private String DB_NAME = "ljcode.db"; //默认数据库
    ...
    private SQLiteUT(Context context) {
        this.context = context;
    }

    private SQLiteUT(Context context, int DB_VERSION, String DB_NAME) {
        this.context = context;
     ...
     * @param context 上下文
     */
    public static SQLiteUT getInstance(Context context) {
        if (sqLiteUT == null) {
            sqLiteUT = new SQLiteUT(context);
        }
        return sqLiteUT;
    }
     ...
     */
    public static SQLiteUT getInstance(Context context, int DB_VERSION, String DB_NAME) {
        sqLiteUT = new SQLiteUT(context, DB_VERSION, DB_NAME);
        return sqLiteUT;
    }
    ...
    private class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            for (SQLTable sqlTable : sqLiteTables) {
                if (!ValidityUT.getInstance().isEmpty(sqlTable.getTableName()) && !ValidityUT.getInstance().isEmpty(sqlTable.getCreateSQL())) {
