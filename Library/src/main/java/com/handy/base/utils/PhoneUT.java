 ...
 */
public class PhoneUT {
    private static PhoneUT phoneUT = null;

    private PhoneUT() {
    }

    public synchronized static PhoneUT getInstance() {
        if (phoneUT == null) {
            phoneUT = new PhoneUT();
        }
        return phoneUT;
    ...
    public void sendSms(Context context, String phoneNumber, String content) {
        Uri uri = Uri.parse("smsto:" + (ValidityUT.getInstance().isEmpty(phoneNumber) ? "" : phoneNumber));
        Intent intent = new Intent(Intent.ACTION_SENDTO, uri);
        intent.putExtra("sms_body", ValidityUT.getInstance().isEmpty(content) ? "" : content);
    ...
    public List<HashMap<String, String>> getAllContactInfo(Context context) {
        ...
        while (cursor.moveToNext()) {
            ...
            //判断contact_id是否为空
            if (!ValidityUT.getInstance().isEmpty(contact_id)) { //null   ""
    ...
    public void getContantNum() {
        PrintfUT.getInstance().LogI("U should copy the following code.");
        /*
